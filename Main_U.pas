...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        MessageDlg(
          'The entered username is too short, it must be atleast 4 letters',
          mtError, [mbOk], 0);
        result := False;
        exit;
      end;
    ...
    begin
      ...
      begin
        ...
        if (sSurname[I] in ['a' .. 'z']) or (sSurname[I] in ['''', '-', '.'])
          then
        begin
          inc(iLowSurname);
        end;
      ...
      begin
        MessageDlg('Please capitilise your surname correctly', mtError, [mbOk],
          0);
        result := False;
        exit;
      end;
      ...
      begin
        MessageDlg('The entered name has (a) foreign character(s) in it',
          mtError, [mbOk], 0);
        result := False;
        exit;
      end;
    ...
    begin
      ...
      begin
        MessageDlg
          ('Your password is too short, it must be 8 or more characters', mtError, [mbOk], 0);
        result := False;
        exit;
      end;
      ...
      begin
        MessageDlg('Your password must have atleast one number', mtError,
          [mbOk], 0);
        result := False;
        exit;
      end;
      ...
      begin
        MessageDlg('Your password must have atleast 1 uppercase letter',
          mtError, [mbOk], 0);
        result := False;
        exit;
      end;
      if (iLowerLetterCount + iCapLetterCount + iNumberCount) <> length
        (sPassword) then
      begin
        MessageDlg('Your password may only consist of letters and number',
          mtError, [mbOk], 0);
        result := False;
        exit;
      end;
    ...
    procedure TfrmMain.MoreInfoTutor(sUsername: string;
      var sFirstname, sSurname: string;
      var iScheduledsessions: integer; var bACtive: boolean);
    begin
      tblTutors.First;
